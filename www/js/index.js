...
(function() {
    ...
    function setup() {
        ...
        $("#Login").on('click', function () {
            ...
            if (username !== "" && password !== "") {
                console.log('Validating Data from Azure');
                if (useOfflineSync) {
                    syncLocalTable().then(validation);
                } else {
                    validation();
                } 
            } else {
    ...
    function validation() {
        ...
        todoItemTable
            .where({ sUSR_logInName: username, sUSR_Password: password })     // Set up the query
            .read()                         // Read the results
            .then(validateUser, handleError);
    }
    /**
  * Create a list of Todo Items
  * @param {System_Users[]} items an array of todoitem objects
  * @returns {void}
  */
    function validateUser(items) {
        console.log("Reading from Azure is sucessfull");
        console.log(items);
